<script setup>
// 1. Charge tous les fichiers JSON du dossier d'un coup ({ eager: true } lit directement le contenu)
const toolsModules = import.meta.glob('../static/tools/*.json', { eager: true })

// 2. Extrait les données de chaque fichier pour créer un tableau d'objets
const toolList = Object.values(toolsModules).map(file => file.default || file)

// 3. (Corrigé) Applique tes filtres. Attention à la parenthèse après computed( !
/* const filteredToolList = computed(() => {
  return toolList.filter(tool => tool.name && tool.description?.trim() !== "")
}) */
</script>
